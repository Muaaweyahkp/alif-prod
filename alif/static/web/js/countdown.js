function triggerCountdown(countdownElem, days, hours, minutes, seconds) {
    // Function to set the countdown duration using Date objects or data attribute
    function setCountdown() {
        const now = new Date().getTime(); // Current time in milliseconds

        // Get end time from data attribute if available
        const countdownWrapper = document.querySelector(countdownElem + "-wrapper");
        if (countdownWrapper && countdownWrapper.dataset.endTime) {
            return new Date(countdownWrapper.dataset.endTime).getTime();
        } else {
